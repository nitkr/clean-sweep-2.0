...
function clean_sweep_display_plugins_tab_content($plugin_results) {
    ...
    if (!$wpmu_dev_available && !empty($plugin_results['wpmu_dev_plugins'])) {
        ...
        if (!empty($plugin_results['wp_org_plugins'])) {
            echo '<h4 style="display:flex; align-items:center; justify-content:space-between;">📦 WordPress.org Plugins to be Re-installed (' . $repo_count . ') <button onclick="copyPluginList(\'reinstall\')" style="background:#007bff;color:white;border:none;padding:4px 8px;border-radius:3px;cursor:pointer;font-size:12px;">Copy</button></h4>';
        ...
        if (!empty($plugin_results['wpmu_dev_plugins'])) {
            ...
            echo '<h4 style="display:flex; align-items:center; justify-content:space-between;">💎 WPMU DEV Premium Plugins to be Re-installed (' . $wpmu_count . ') <button onclick="copyPluginList(\'wpmudev\')" style="background:#7c3aed;color:white;border:none;padding:4px 8px;border-radius:3px;cursor:pointer;font-size:12px;">Copy</button></h4>';
